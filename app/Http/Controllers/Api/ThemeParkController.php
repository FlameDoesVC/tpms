...
class ThemeParkController extends Controller
{
    ...
    public function updateSlot(Request $request, EventSlot $slot): JsonResponse
    {
        Gate::authorize('update', $slot->event);

        $validated = $request->validate([
            'slot_date' => ['sometimes', 'date'],
            'slot_time' => ['sometimes', 'date_format:H:i'],
    ...
    public function destroySlot(EventSlot $slot): Response
    {
        Gate::authorize('update', $slot->event);

        if ($slot->template_id !== null) {
            throw ValidationException::withMessages([
                'slot' => 'This slot was generated by a recurring schedule - cancel it instead of deleting, or the next generation run will recreate it.',
            ]);
        }
